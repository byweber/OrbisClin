...
- **StoneWebViewer / OHIF:** para imagens com necessidade diagnóstica formal (dermoscopia alta resolução) — complementar à timeline OrbisClin, não substituto
...
3. **Respeite §8** (imports via `app.core`, erros, auditoria, CSS, worker lazy)
...
*Última atualização: upload estável com fallback síncrono para extração de texto; visualização de arquivos via cookie; modal de upload com lookup automático de paciente e accession; integração Orthanc no roadmap*
